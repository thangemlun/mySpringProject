...
<c:url var="newsAPIwithImage" value ="/api/news/upLoadImage" />
...
<html>
   ...
   <body>
            ...
            <div class="page-content">
               <div class="row">
                  <div class="col-xs-12">
                     ...
                     <form:form class="form-horizontal" role="form" id="formSubmit" modelAttribute="model" method="post">
                     <div class="form-group">
                        ...
                        <div class="form-group">
                        <label class="col-sm-3 control-label no-padding-right" for="imageFile"> Ảnh đại diện  </label>
                          <div class="col-sm-9">
                          	  <form:input path="thumbnail" id="thumbnail" cssClass="col-xs-10 col-sm-5" placeholder="Nhúng đường link ảnh  ..." />
                          	   <br>
                          	   <br>
                          	   <label for="imageFile">Hoặc chọn file bên dưới</label>
      ...
      $('#btnAddOrUpdateNew').click(function(e) {
    	  e.preventDefault();
    	  var file_data = $("#imageFile")[0].files[0];
    	  var form = $('#formSubmit')[0];
    	  var dataNewsWithImage = new FormData(form);
    	  dataNewsWithImage.append("imageFile",file_data);
    	...
    	//lấy dữ liệu từ các field theo name của field trong form với form có id đã xác định là $('#formSubmit').
		  ...
		  dataNewsWithImage.append("news",JSON.stringify(data));
			  	...
			  	if(id == "")
			  		{
			  			if( file_data.size >= 1)
			  				{
			  					addNewsWithImage(dataNewsWithImage);
			  				}else {
			  					addNews(data);
			  				}
				  	} 
				  	else {
				  		if(id != '' && file_data.size >= 1)
				  			{
				  			/* console.log(dataNewsWithImage.get('news')); */
				  			 updateNewsWithImage(dataNewsWithImage); 
				  			}else {
      ...
      function addNewsWithImage(data){
    	  $.ajax({
    		  url:'${newsAPIwithImage}',
    		  type: "POST",
    	      data:	data,
      ...
      function updateNewsWithImage(data){
    	  $.ajax({
    		  url:'${newsAPIwithImage}',
    		  type: "POST",
    	      data:	data,
    		  ...
    		  cache: false,
    		  success: function (result) {
					window.location.href = "${editNewsUrl}?&message=update_success";
				},
			  error: (error) => {
				   window.location.href = "${newsURL}?page=1&limit=2&message=error_system"; 
			  }
