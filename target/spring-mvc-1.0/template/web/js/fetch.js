	...
	if(event.target.readyState === 'interactive'){
		...
		let displayResults = (results) => {
			let htmlString = results.map((result)=> {
				...
				return `<li class="list-group-item"><c:url var="newsItemURL" value="/bai-viet"><c:param name="title" value="${result.title}"/><c:param name="page" value="1"/><c:param name="limit" value="3"/></c:url><a href="${itemUrl}" class="list-group-item list-group-item-action flex-column align-items-start"><div class="d-flex w-100 justify-content-between"><h5 class="mb-1" style="white-space:nowrap;width:80%;overflow:hidden; text-overflow:ellipsis">${result.title}</h5><small>${newsDatePosted}</small></div><p class="mb-1" style="white-space:nowrap;width:80%;overflow:hidden; text-overflow:ellipsis" >${result.shortDescription}</p><img src="${result.thumbnail}" width="40%" /></a></li>`
			}).join('');
