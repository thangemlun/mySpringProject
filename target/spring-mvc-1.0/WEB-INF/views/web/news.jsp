        ...
        <security:authorize access = "isAuthenticated()"> 
         ...
         <!--  Reply cmt -->
			...
			<div class="container">
				<div class="row">
					<div class="col-md-8">
						<h2 class="page-header"> <i class="fa fa-comments" aria-hidden="true"></i> Comments</h2>
						<c:forEach var="item" items="${listComment.listResult}"> 
							...
							<div class="row">
								...
								<div class="col-md-10 col-sm-10">
										...
										<div class="panel-body">
											<header class="text-left">
												<div class="comment-user">
													<i class="fa fa-user-circle-o" aria-hidden="true"></i> <strong> ${item.userName}</strong>
												</div>
											...
											<br>
											<div class="comment-user">
													<i class="fa fa-comment"></i><strong> ${item.content}</strong>
												</div>
											<%-- <div class="comment-post">
											<i class="fa fa-comment"></i><p>${item.content}</p>
											</div> --%>
											<p class="text-right">
												<a class="btn btn-default btn-sm" id="btnReply_${item.id}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>  reply</a>
											</p>
											<security:authorize access = "isAnonymous()">
					    						<form class="form-horizontal" role="form" id="formRepliedComment_${item.id}"> 
										         <div class="post-comment">
									...
									<c:if test='${item.countRepliedComments != 0}'>
										<a class="btn btn-default btn-sm" id="showRepliesList_${item.id}"><i class="fa fa-share-square-o" aria-hidden="true"></i> Bình luận có ${item.countRepliedComments} trả lời </a>
									</c:if>
											...
											<c:forEach var="repliedItem" items="${item.listRepliedComment}">	
												<div class="media text-muted pt-3">
													<p
														...
														<strong class="d-block text-gray-dark"> <i class="fa fa-user" aria-hidden="true"></i> ${repliedItem.userName} - <small class="text-muted"><i class="fa fa-clock-o" aria-hidden="true"></i> ${repliedItem.timeCommentPosted} </small> </strong>
														${repliedItem.content}
													</p>
												</div>
											</c:forEach>
